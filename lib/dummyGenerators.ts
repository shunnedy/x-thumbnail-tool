...
function generateDummy(type: DummyType): HTMLCanvasElement {
  ...
  switch (type) {
    ...
    case 'ocean':    drawOcean(ctx, W, H);    break;
    case 'forest':   drawForest(ctx, W, H);   break;
    case 'mountain': drawMountain(ctx, W, H); break;
    case 'desert':   drawDesert(ctx, W, H);   break;
    case 'flowers':  drawFlowers(ctx, W, H);  break;
    case 'aurora':   drawAurora(ctx, W, H);   break;
    case 'city':     drawCity(ctx, W, H);     break;
    case 'bamboo':   drawBamboo(ctx, W, H);   break;
    case 'clouds':   drawClouds(ctx, W, H);   break;
  }
  return c;
}
...
function drawSunset(ctx: CanvasRenderingContext2D, W: number, H: number) {
  ...
  // Sun glow
  const glow = ctx.createRadialGradient(W/2, H*0.72, 0, W/2, H*0.72, H*0.6);
  ...
  // Sun disc
  ctx.beginPath(); ctx.arc(W/2, H*0.72, H*0.055, 0, Math.PI*2);
  ...
  ctx.bezierCurveTo(W*0.15, H*0.65, W*0.32, H*0.73, W*0.5, H*0.68);
  ctx.bezierCurveTo(W*0.68, H*0.63, W*0.82, H*0.72, W, H*0.76);
...
function drawOcean(ctx: CanvasRenderingContext2D, W: number, H: number) {
  ...
  // Horizon shimmer
  const shim = ctx.createLinearGradient(0, H*0.41, 0, H*0.44);
  shim.addColorStop(0, 'rgba(255,255,255,0.5)'); shim.addColorStop(1, 'transparent');
  ctx.fillStyle = shim; ctx.fillRect(0, H*0.41, W, H*0.04);

  // Wave lines
  for (let i = 0; i < 14; i++) {
    const y = H*0.47 + i * H*0.038;
  ...
  // Sun reflection
  const ref = ctx.createLinearGradient(W*0.4, H*0.42, W*0.6, H);
  ref.addColorStop(0, 'rgba(255,230,120,0.35)'); ref.addColorStop(1, 'transparent');
  ctx.fillStyle = ref; ctx.fillRect(W*0.4, H*0.42, W*0.2, H*0.58);
}
...
function drawForest(ctx: CanvasRenderingContext2D, W: number, H: number) {
  ...
  ctx.fillStyle = '#0e2114'; ctx.fillRect(0, H*0.2, W, H*0.8);

  // Far trees (lighter layer)
  ctx.fillStyle = '#1a4025';
  for (let i = 0; i < 12; i++) {
    const x = (i * W/10) + Math.sin(i*2.3)*20;
    const h = H*0.38 + Math.sin(i*1.7)*H*0.06;
    drawTree(ctx, x, H*0.55, W*0.04, h);
  }

  // Near trees (dark layer)
  ctx.fillStyle = '#0d1f0f';
  for (let i = 0; i < 9; i++) {
    const x = (i * W/7.5) - W*0.05;
    const h = H*0.55 + Math.sin(i*2.1)*H*0.08;
    drawTree(ctx, x, H, W*0.06, h);
  }

  // Ground (dark)
  const gnd = ctx.createLinearGradient(0, H*0.85, 0, H);
  gnd.addColorStop(0, '#0a1a0c'); gnd.addColorStop(1, '#040c05');
  ctx.fillStyle = gnd; ctx.fillRect(0, H*0.85, W, H*0.15);
...
function drawMountain(ctx: CanvasRenderingContext2D, W: number, H: number) {
  ...
  for (const m of mountains) {
    ...
    ctx.lineTo(0, m.peaks[0][1]*H);
    for (let i = 1; i < m.peaks.length; i++) {
      const [x,y] = m.peaks[i];
      const [px, py] = m.peaks[i-1];
      ctx.quadraticCurveTo((px+x)/2*W, (py+y)/2*H - H*0.05, x*W, y*H);
    }
...
function drawDesert(ctx: CanvasRenderingContext2D, W: number, H: number) {
  ...
  // Sun
  ctx.beginPath(); ctx.arc(W*0.72, H*0.18, H*0.05, 0, Math.PI*2);
  ctx.fillStyle = 'rgba(255,245,180,0.97)'; ctx.fill();

  // Sand
  const sand = ctx.createLinearGradient(0, H*0.45, 0, H);
  sand.addColorStop(0, '#d4924a'); sand.addColorStop(0.5, '#c07830'); sand.addColorStop(1, '#8a5015');
  ctx.fillStyle = sand; ctx.fillRect(0, H*0.45, W, H*0.55);

  // Dunes
  ctx.fillStyle = '#b8662a';
  ctx.beginPath(); ctx.moveTo(0, H*0.62);
  ctx.bezierCurveTo(W*0.25, H*0.45, W*0.4, H*0.58, W*0.65, H*0.52);
  ctx.bezierCurveTo(W*0.8, H*0.47, W*0.9, H*0.55, W, H*0.6);
  ctx.lineTo(W,H); ctx.lineTo(0,H); ctx.closePath(); ctx.fill();

  ctx.fillStyle = '#9e5420';
  ctx.beginPath(); ctx.moveTo(0, H*0.8);
  ctx.bezierCurveTo(W*0.2, H*0.7, W*0.45, H*0.78, W*0.6, H*0.72);
  ctx.bezierCurveTo(W*0.75, H*0.67, W*0.9, H*0.76, W, H*0.8);
  ctx.lineTo(W,H); ctx.lineTo(0,H); ctx.closePath(); ctx.fill();

  // Heat shimmer at horizon
  const shim = ctx.createLinearGradient(0, H*0.43, 0, H*0.52);
  shim.addColorStop(0, 'transparent'); shim.addColorStop(0.5, 'rgba(255,200,100,0.2)'); shim.addColorStop(1, 'transparent');
  ctx.fillStyle = shim; ctx.fillRect(0, H*0.43, W, H*0.1);
}

function drawFlowers(ctx: CanvasRenderingContext2D, W: number, H: number) {
  // Grass background
  ...
  for (let i = 0; i < 80; i++) {
    const x = Math.random()*W, y = H*0.1 + Math.random()*H*0.85;
...
function drawAurora(ctx: CanvasRenderingContext2D, W: number, H: number) {
  ...
  for (const b of bands) {
    for (let x = 0; x <= W; x += 4) {
      const wave = Math.sin(x/W*Math.PI*3 + 1.2)*H*0.06;
      const wave2 = Math.cos(x/W*Math.PI*2.5)*H*0.04;
      const cy = b.y*H + wave + wave2;
      const grad = ctx.createLinearGradient(x, cy-H*b.spread, x, cy+H*b.spread);
      ...
      ctx.fillStyle = grad;
      ctx.fillRect(x, cy-H*b.spread, 4, H*b.spread*2);
    }
  }
  ...
  ctx.bezierCurveTo(W*0.2, H*0.82, W*0.5, H*0.9, W*0.7, H*0.84);
  ctx.bezierCurveTo(W*0.85, H*0.79, W*0.95, H*0.86, W, H*0.88);
  ctx.lineTo(W,H); ctx.lineTo(0,H); ctx.closePath(); ctx.fill();
}
...
function drawCity(ctx: CanvasRenderingContext2D, W: number, H: number) {
  ...
  const glow = ctx.createRadialGradient(W/2, H*0.72, 0, W/2, H*0.72, H*0.55);
  ...
  // Buildings silhouettes (scale x/w by W/1080, h by H)
  ...
  ].map(b => ({ x: b.x * W/1080, w: b.w * W/1080, bh: b.h * H }));

  ctx.fillStyle = '#080808';
  for (const b of buildingConfigs) {
    ctx.fillRect(b.x, H - b.bh, b.w, b.bh);
    // Windows
    for (let wy = H - b.bh + 8; wy < H - 12; wy += 18) {
...
function drawBamboo(ctx: CanvasRenderingContext2D, W: number, H: number) {
  ...
  for (const stalk of stalks) {
    ...
    ctx.fillStyle = sg; ctx.fillRect(stalk.x, 0, stalk.w, H);

    // Nodes
    for (let y = 80; y < H; y += 90+Math.random()*40) {
...
function drawClouds(ctx: CanvasRenderingContext2D, W: number, H: number) {
  // Sky gradient
  const sky = ctx.createLinearGradient(0, 0, 0, H);
  ...
  // Sun
  const sunGrad = ctx.createRadialGradient(W*0.15, H*0.12, 0, W*0.15, H*0.12, H*0.1);
  ...
  for (const cd of cloudDefs) {
    drawCloud(ctx, cd.x*W, cd.y*H, cd.scale);
  }
